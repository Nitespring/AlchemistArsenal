...
public class FireBottleItem extends Item implements ProjectileItem {
    public FireBottleItem(Properties pProperties) {
        super(pProperties);
    }

    @Override
    public Projectile asProjectile(Level pLevel, Position pPos, ItemStack pStack, Direction pDirection) {
        FireBottle bottle = new FireBottle(EntityInit.FIRE_BOTTLE.get(),pLevel);
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide) {
            FireBottle bottle = new FireBottle(EntityInit.FIRE_BOTTLE.get(), level);
            bottle.setOwner(player);
            bottle.setPos(pos.add(aim.x()*0.5f,1.75f+aim.y()*0.5f,aim.z()*0.5f));
            bottle.shootFromRotation(player, player.getXRot(), player.getYRot(), -20.0F, 0.75F, 1.0F);
