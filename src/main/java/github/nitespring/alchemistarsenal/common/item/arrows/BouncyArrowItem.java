...
public class BouncyArrowItem extends ArrowItem implements ProjectileItem {
    ...
    public AbstractArrow createArrow(Level level, ItemStack stack, LivingEntity e, @Nullable ItemStack stack1) {
        AbstractArrow arrow = new BouncyArrow(e, level, stack.copyWithCount(1),stack1);
        arrow.setBaseDamage(basedamage);
        return arrow;
    }
    @Override
    public Projectile asProjectile(Level pLevel, Position pPos, ItemStack pStack, Direction pDirection) {
        AbstractArrow arrow = new BouncyArrow(pPos.x(), pPos.y(), pPos.z(),pLevel, pStack.copyWithCount(1), null);
        arrow.setBaseDamage(basedamage);
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltipComponents, TooltipFlag tooltipFlag) {
        tooltipComponents.add(Component.translatable("comment.alkhars.bouncy_arrow").withStyle(ChatFormatting.GRAY));
